...
;регистр a пустой
clear_loop: ;цыкл для очистки дисплея
  st a, d ;стираем байт на дисплее
  inc d ;переходи к следующему байту
  jns b ;если не вышли за пределы дисплея, то продолжаем очистку
ldi a, 0b00011100 ;выводим змейку
st a, 0x7a
ld a, head_pos ;выводим голову змейки
st a, 0x5a
;регистр c содержит 0 (1 банк)
;регистр d содержит адрес перехода (0x80)
set_bank:
  st c, bank ;сохраняем банк
  jmp d ;переходим к следующему блоку
;после съедения яблока переходи к генерации нового
other_bank:
  clr c
  st c, bank
  jmp apple_random
...
last_cell_pos db 4 ;кол-во итераций для выравнивания направления клетки змейки
last_key db KEY_RIGHT ;последние направление змейки (изначально вправо)
...
display_b   db  0b00000000, 0b00000000,
                ...
                0b00000000, 0b00000000,
                0b00000000, 0b00000000
;BANK 1: BANK START - отвечает за 1 генерацию яблока и чтение клавиатуры
apple_random:
  ld a, score ;смотрим, не превысила ли длина 200
  ldi b, 256 - 200
  add a, b
  jnc apple_random1 ;если нет, то генерируем яблоко 1 способом
...
apple_random1:
  ...
  ldi c, 0x40
  add a, c
  ;проверка на попадение яблока на змейку
  mov d, a
  shr c ;0x20
...
add_score:
  ...
  inc a
  st a, score
  ;если длина стала 255, то переходи к победе
  not a
  jnz add_last_cell
  ldi c, 5
  ldi d, win_start
...
add_last_cell:
  ...
  dec a ;уменьшаем кол-во сдвигов на 2
  dec a
  jns save_last_cell ;если число неотрицательное, то сохраняем позицию
  ldi a, 6 ;если отрицательное, то обновляем кол-во сдвигов
...
key_read:
  ...
  ld b, lock_key ;если она запретная, читаем заного
  xor b, a
  jz key_read + 2
  ;клавиша вправо
  ...
  xor b, a
  jz down
  ;если введена не одна из доступных клавиш, то проверяем последнюю
  ld a, last_key
  jmp key_read + 9
;регистр c уже содержит 2 (2 банк)
right:
...
right_start:
  ...
  jnc next_jmp ;если не вышли за рамки, то переходи к сохранению
  ...
  inc b
  jmp next_jmp
;тоже самое, что и для правой, но наоборот
left_start:
  st a, last_key
  ldi a, KEY_RIGHT
  st a, lock_key
...
up_start:
  ...
  ldi c, 0x60 ;проверяем на столкновение с верхней стеной
  xor c, b
  shr c
  jz game_over
  dec b ;сдвиагаем адрес вверх
  dec b
  jmp next_jmp
;тоже самое что и для верхней, но наоборот
down_start:
  st a, last_key
  ldi a, KEY_UP
  st a, lock_key
  ...
  inc b
next_jmp:
  ;проверяем на столкновение со хвостом
  ld c, b
  and c, a
  jnz game_over
  ...
  st b, head_adress
  ld c, b ;выводи синюю часть головы на дисплей
  or c, a
  st c, b
  ldi c, 3
  ldi d, next_jmp2
...
;отвечает за обработку коллисий с яблоком, перемещение хвоста
;и 2 генерацию яблока
;BANK COLLISION
next_jmp2:
  ldi c, 0x20 ;выводи красную часть головы на дисплей
  sub b, c
  ld c, b
  or c, a
  st c, b
check_collision_apple:
  ...
  jnz jmp_bank_4
eating_apple:
  ;если есть столкновение, то меняем адрес перехода
  ;после сохранения направления головы и переходи к ней
  ldi a, other_bank
  st a, next
  ldi c, 4
  ...
  jmp set_bank
jmp_bank_4:
  ;если столкновения нет, то восстанавливаем адрес перехода
...
new_tail2:
  ...
  ld b, tail_adress ;заранее читаем адрес звоста
  ...
  ;право
tail_right:
  shr a ;сдвигаем поизицию вправо
  jnc d ;если перехода между байтами нет, то переходи к сохранению
  ldi a, 0b10000000 ;если переход есть, то перемещаем позицию в левый край
  inc b ;сдвигаем адрес вправо
  jmp d ;переходи к сохранению
tail_up:
...
save_tail:
  ...
  clr c ;переходи на чтение клавши
  ldi d, key_read
  jmp set_bank
;BANK RANDOM 2
apple_random2:
...
random_loop1:
  ...
  random_loop2:
    ...
    and c, b ;если попали на хвост змейки, то переходи в конец
    jnz end_random2
    dec d ;если нет, то убавляем счётчик
    js end_random ;если счётчик ниже нуля, то выходи из цикла
  end_random2:
    shr b ;сдвигаем позицию впараво
    jnc random_loop2 ;если не сдвинули за край байта, то начинаем сначала
  jmp random_loop1 ;если сдвинули, то смещаем адрес
end_random:
  ldi c, 0x20 ;переходи на красный дисплей
  sub a, c
  clr c ;переходи на сохранение яблока на дисплей
...
delete_tail:
  ...
  st c, b
new_tail:
  ;сохраняем следующие напраление хвоста
  ld a, snake
  clr b
  shl a
  rcl b
...
loop3_start:
  ldi c, snake ;вычисляем кол-во итераций цикла
  sub b, c
loop3:
